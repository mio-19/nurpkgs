...
# 2.4.20-dev (nixpkgs Kotlin 2.2.20 cannot compile this tree).
...
find platform/jewel -type f -name '*.iml' -exec sed -i \
  ...
  -e 's/JVM 25/JVM 24/g' \
  -e 's/JVM \[25\]/JVM [24]/g' \
  -e 's/name="languageVersion" arg="[0-9.]*"/name="languageVersion" arg="1.9"/g' \
  -e 's/name="apiVersion" arg="[0-9.]*"/name="apiVersion" arg="1.9"/g' \
  {} +
